...
function Goals() {
  ...
  return <Layout><h1>Financial Goals</h1><section className="card"><h2>{editing ? 'Edit goal' : 'Create a spending goal'}</h2><form className="form-grid" onSubmit={submit}>{error && <p className="error full">{error}</p>}<label>Goal name<input required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Food budget" /></label><label>Spending limit<input type="number" min="0.01" step="0.01" required value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} /></label><label>Period<select value={form.period} onChange={e => setForm({ ...form, period: e.target.value })}><option value="DAILY">Daily</option><option value="WEEKLY">Weekly</option><option value="MONTHLY">Monthly</option></select></label><div className="full"><button>{editing ? 'Save changes' : 'Create goal'}</button>{editing && <button type="button" className="secondary" onClick={() => { setEditing(null); setForm(blankGoal); }}>Cancel</button>}</div></form></section><section className="card"><h2>Your goals</h2>{savedGoals.length ? <div className="goal-list">{savedGoals.map(goal => { const period = String(goal.period || 'MONTHLY'), spent = spentFor(period), percent = Math.min(100, (spent / Number(goal.amount)) * 100); return <article className="goal" key={goal.id}><div><h3>{goal.name}</h3><p>{money(spent)} of {money(goal.amount)} spent this {period.toLowerCase().replace('ly', '')}</p><div className="progress"><span className={spent > Number(goal.amount) ? 'over' : ''} style={{ width: `${percent}%` }} /></div>{spent > Number(goal.amount) && <p className="error">This goal has been exceeded.</p>}</div><div><button className="text-button" onClick={() => { setEditing(goal.id); setForm({ name: goal.name, amount: goal.amount, period }); }}>Edit</button><button className="text-button danger" onClick={() => remove(goal.id)}>Delete</button></div></article>; })}</div> : <p>No goals yet.</p>}</section></Layout>;
}
